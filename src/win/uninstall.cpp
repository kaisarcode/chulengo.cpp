 ...
 * Summary: Native uninstaller logic for the chulengo runtime on Windows.
 *
 * Author:  KaisarCode
...
#define KC_UNINSTALLER_TITLE "chulengo uninstaller"
...
static int kc_remove_app(const struct kc_config *config) {
    ...
    if (PathCombineA(bin_path, config->install_root, "bin\\chulengo.exe") == NULL) {
        return 1;
    }
