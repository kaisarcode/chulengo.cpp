...
stage_local_assets() {
    ...
    mkdir -p "$stage_dir/llama.cpp/$arch" "$stage_dir/ggml/$arch"
    cp -a "./lib/obj/llama.cpp/$arch/." "$stage_dir/llama.cpp/$arch/"
...
stage_remote_assets() {
    ...
    find_remote_assets "llama.cpp" "$arch" | while IFS="$(printf '\t')" read -r asset_mode asset_name asset_blob_url; do
        [ -n "$asset_name" ] || continue
        if [ "$asset_mode" = "120000" ]; then
            ln -s "$(download_blob_text "$asset_blob_url")" "$stage_dir/llama.cpp/$arch/$asset_name"
            continue
        fi
        download_asset "$CORE_REPO_ROOT/lib/obj/llama.cpp/$arch/$asset_name" \
            "$stage_dir/llama.cpp/$arch/$asset_name"
    done
